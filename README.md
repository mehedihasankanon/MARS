...
MARS follows a decoupled, client-server architecture:
- **Client Tier:** A Next.js frontend that handles routing, server-side rendering (where applicable), and UI state. 
- **API Tier:** A monolithic Node/Express REST API that securely manages business logic, media uploads, and request validation.
- **Data Tier:** A PostgreSQL database enriched with triggers, stored procedures, and complex relational constraints to ensure atomicity and consistency for transactional data like orders and payments.

---

## ✨ Features

MARS handles the complete e-commerce ecosystem, split into logical modules:

### Product & Catalog Management
- **Hierarchical Categories & Varieties:** Robust classification system for inventory.
- **Dynamic Catalog:** Advanced product filtering, searching, and pagination.
...
- **Checkout Flow:** Complex order generation handling cart-to-order conversions.
- **Payments:** Payment gateway integration placeholders and tracking schemas.
- **Offers & Coupons:** Discount calculation engine supporting percentage-based and fixed promotional codes.

### Post-Purchase Lifecycle
- **Shipment Tracking:** Logistics tracking from dispatch to delivery.
- **Returns & Reports:** Automated return request flows and system-level issue reporting.
...
- **Analytics:** Data aggregation endpoints to drive admin dashboards and seller insights.

---

## 🔐 Security Features

Security was treated as a first-class citizen in the development of MARS:
...
- **Concurrent Tooling:** Uses `concurrently` for efficient development server execution, bridging frontend and backend processes.
- **Asset Offloading:** Images and media are stored on **Cloudinary**, significantly reducing server bandwidth and ensuring scalable asset delivery.
- **Stateless Architecture:** The backend API is primarily stateless (via JWTs), allowing it to scale horizontally behind a load balancer in the future.
...
- Backend API will be available at: `http://localhost:5000` (or as configured)

---

## ⚙️ Environment Variables

### Root / Backend `.env` parameters:
...
│   ├── database/            # Schema definitions and seeding scripts
...
This project is open-source and available under the terms of the [LICENSE](./LICENSE) included in this repository.
