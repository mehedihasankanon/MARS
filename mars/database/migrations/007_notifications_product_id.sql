-- 7. Normalize notifications with order context and notification type

-- Extend notifications so order notifications can deep-link into the dashboard.
ALTER TABLE mars.Notifications
ADD COLUMN IF NOT EXISTS Order_ID UUID REFERENCES mars.Orders(Order_ID) ON DELETE SET NULL,
ADD COLUMN IF NOT EXISTS Notification_Type VARCHAR(20);

-- Order notifications
CREATE OR REPLACE FUNCTION mars.notify_seller_on_order()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
  v_seller_id UUID;
  v_product_name VARCHAR;
BEGIN
  SELECT Seller_ID, Name INTO v_seller_id, v_product_name
  FROM mars.Products
  WHERE Product_ID = NEW.Product_ID;

  INSERT INTO mars.Notifications (User_ID, Message, Product_ID, Order_ID, Notification_Type)
  VALUES (
    v_seller_id,
    'New order received for your product: ' || v_product_name || ' (Qty: ' || NEW.Quantity || ')',
    NEW.Product_ID,
    NEW.Order_ID,
    'order'
  );

  RETURN NEW;
END;
$$;

-- Question notifications
CREATE OR REPLACE FUNCTION mars.notify_seller_on_question()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
  v_seller_id UUID;
  v_product_name VARCHAR;
BEGIN
  SELECT Seller_ID, Name INTO v_seller_id, v_product_name
  FROM mars.Products
  WHERE Product_ID = NEW.Product_ID;

  INSERT INTO mars.Notifications (User_ID, Message, Product_ID, Notification_Type)
  VALUES (
    v_seller_id,
    'A new question was asked on your product: ' || v_product_name,
    NEW.Product_ID,
    'question'
  );

  RETURN NEW;
END;
$$;

-- Review notifications
CREATE OR REPLACE FUNCTION mars.notify_seller_on_review()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
  v_seller_id UUID;
  v_product_name VARCHAR;
BEGIN
  SELECT Seller_ID, Name INTO v_seller_id, v_product_name
  FROM mars.Products
  WHERE Product_ID = NEW.Product_ID;

  INSERT INTO mars.Notifications (User_ID, Message, Product_ID, Notification_Type)
  VALUES (
    v_seller_id,
    'A new review was posted on your product: ' || v_product_name || ' (Rating: ' || NEW.Rating || '/5)',
    NEW.Product_ID,
    'review'
  );

  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_notify_seller_on_review ON mars.Reviews;
CREATE TRIGGER trg_notify_seller_on_review
AFTER INSERT ON mars.Reviews
FOR EACH ROW
EXECUTE FUNCTION mars.notify_seller_on_review();

-- Backfill notification type for existing rows that pre-date Notification_Type.
UPDATE mars.Notifications
SET Notification_Type = CASE
  WHEN Message ILIKE 'New order received%' THEN 'order'
  WHEN Message ILIKE 'A new question was asked%' THEN 'question'
  WHEN Message ILIKE 'A new review was posted%' THEN 'review'
  ELSE 'general'
END
WHERE Notification_Type IS NULL;

