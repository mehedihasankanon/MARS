...
export default function ProductDetailPage() {
  ...
  const handleSubmitQuestion = async (e) => {
    ...
    try {
      await api.post(`/questions/${params.id}`, { questionText: questionText.trim() });
      setQuestionText('');
  ...
  const handleSubmitAnswer = async (questionId) => {
    ...
    try {
      await api.put(`/questions/${params.id}/${questionId}/answer`, { answerText: text.trim() });
                ...
                <>
                  ...
                  >
                    {product.images.map((img, idx) => (
                      <div key={img.image_id} className="w-full h-full flex-shrink-0 flex items-center justify-center">
                        <img
                          src={img.image_url}
          ...
          <div className="flex flex-col">
            ...
            {parseFloat(product.avg_rating) > 0 && (
              <div className="flex items-center gap-2 mb-4">
                <div className="flex">
                  {[1, 2, 3, 4, 5].map(star => (
                    <svg key={star} className={`w-4 h-4 ${star <= Math.round(parseFloat(product.avg_rating)) ? 'text-yellow-400' : 'text-gray-600'}`} fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                  ))}
                </div>
                <span className="text-sm text-gray-400">{parseFloat(product.avg_rating).toFixed(1)}</span>
          ...
          {activeTab === 'reviews' && (
            <div>
                ...
                <div className="bg-[#111111] border border-[#2A2A2A] rounded-xl p-6 mb-6">
                  ...
                  <form onSubmit={handleSubmitReview} className="space-y-4">
                    <div>
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map(star => (
                          ...
                          >
                            <svg className={`w-7 h-7 transition-colors ${star <= reviewForm.rating ? 'text-yellow-400' : 'text-gray-600 hover:text-gray-400'}`} fill="currentColor" viewBox="0 0 20 20">
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                            </svg>
                          </button>
                        ))}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {reviewMessage.text && (
                        <span className={`text-sm ${reviewMessage.type === 'success' ? 'text-green-400' : 'text-red-400'}`}>
                          {reviewMessage.text}
                        </span>
                      )}
                    </div>
              ...
              {reviews.length === 0 ? (
                ...
                <div className="space-y-4">
                  {reviews.map(review => (
                    <div key={review.review_id} className="bg-[#111111] border border-[#2A2A2A] rounded-xl p-5">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-3">
                          {review.profile_picture ? (
                            <img src={review.profile_picture} alt="" className="w-8 h-8 rounded-full object-cover" />
                          ) : (
                          ...
                          <div>
                            ...
                            <p className="text-gray-500 text-xs">{new Date(review.review_date).toLocaleDateString()}</p>
                          </div>
                        </div>
          ...
          {activeTab === 'qa' && (
            <div>
              ...
              {questions.length === 0 ? (
                ...
                <div className="space-y-4">
                  {questions.map(q => (
                    <div key={q.question_id} className="bg-[#111111] border border-[#2A2A2A] rounded-xl p-5">
                      <div className="flex items-start gap-3 mb-2">
                        <span className="text-[#E85D26] font-bold text-sm mt-0.5">Q:</span>
                        <div className="flex-1">
                          <p className="text-white text-sm">{q.question_text}</p>
                          <p className="text-gray-500 text-xs mt-1">
                            Asked by <span className="text-gray-400">{q.customer_username}</span>
                      ...
                      {q.answer_text ? (
                        <div className="flex items-start gap-3 mt-3 pl-6 border-l-2 border-[#E85D26]/30">
                          <span className="text-green-400 font-bold text-sm mt-0.5">A:</span>
                          <div className="flex-1">
                            <p className="text-gray-300 text-sm">{q.answer_text}</p>
                            <p className="text-gray-500 text-xs mt-1">
                              Answered by <span className="text-gray-400">{q.seller_username}</span>
                        ...
                        <p className="text-gray-600 text-xs mt-2 pl-6 italic">Awaiting seller response...</p>
                      )}
                    </div>
                  ))}
                </div>
