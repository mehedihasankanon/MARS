 ...
 * =====================================================================
 *
 * PURPOSE: Displays the logged-in user's account information.
 *          Shows their name, email, role, and provides quick links
 *          to cart, orders, and (for sellers) the dashboard.
 *
 * WHY NO API CALL?
 *   The user data is already available in AuthContext (loaded when
 *   the user logged in or when the page refreshed and the token was
 *   restored). So we just read from useAuth() — no extra API call needed.
 *
 * AUTH GUARD:
 *   If the user is not logged in, we redirect to the login page.
 *   This is done in useEffect — checks after auth loading finishes.
 *
 * LAYOUT:
 * ┌─────────────────────────────────────────────────┐
 * │  Profile Header (avatar + name + role badge)    │
 * ├─────────────────────────────────────────────────┤
 * │  Account Details card (email, username, phone)  │
 * ├─────────────────────────────────────────────────┤
 * │  Quick Actions grid (My Orders, Cart, etc.)     │
 * └─────────────────────────────────────────────────┘
 * =====================================================================
 */
export default function ProfilePage() {
   ...
   * We wait for auth loading to finish before checking,
   * otherwise the user would be immediately redirected
   * before the token restoration has a chance to run.
   */
  useEffect(() => {
    if (!loading && !user) {
  ...
  // Maps role strings to user-friendly labels and colors
  const roleConfig = {
    ...
    admin:    { label: 'Admin',     emoji: '⚙️', color: 'text-purple-400 bg-purple-400/10' },
  };
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {/* ── PROFILE HEADER CARD ──────────────────────────
             Shows avatar initial, full name, and role badge.
             ──────────────────────────────────────────── */}
        <div className="bg-[#111111] rounded-xl border border-[#2A2A2A] p-8">
        ...
        {/* ── ACCOUNT DETAILS CARD ─────────────────────────
             Displays the user's account information in a
             labelled grid format.
             ──────────────────────────────────────────── */}
        <div className="bg-[#111111] rounded-xl border border-[#2A2A2A] p-8">
          <h2 className="text-lg font-bold text-white mb-6">Account Details</h2>
          
          <div className="grid sm:grid-cols-2 gap-6">
            {/* Each detail block: Label (gray) + Value (white) */}
            <div>
        ...
        {/* ── QUICK ACTIONS GRID ───────────────────────────
             Shortcut cards to navigate to related pages.
        ...
        <div className="bg-[#111111] rounded-xl border border-[#2A2A2A] p-8">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            {/* Browse Products — visible to all */}
            <Link
              href="/products"
              className="flex items-center gap-4 p-4 bg-[#1A1A1A] rounded-lg border border-[#2A2A2A] hover:border-[#E85D26]/40 transition-all group"
            >
